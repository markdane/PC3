---
title: "LINCs MEP SS1 Pilot Analysis Pipeline Explanation"
author: "Mark Dane"
date: "`r Sys.Date()`"
output: pdf_document
---
...
##Summary
This experiment uses the LINCs Structure and Morphology staining set on PC3 cells in 8 well MEMAs. 
...
Images of each well were gathered on a Tecan LS Reloaded laser scanner. This staining set includes, DAPI, H3K9me3 (488nm), Fibrillarin(555) and EdU(635 and 647nm). Data from DAPI staining is only gathered by the ScanR. 
...
cDT <- fread("./Annotated Data/LIX800101CellAnn.txt")

sDT <- fread("./Annotated Data/LI8X00101SpotAnn.txt")

barcodes <- sort(unique(popDT$Barcode))[1]
...
DT <-sDT[sDT$Barcode=="LI8X00101" & sDT$Well == "A03"]
#Remove the fiducial entries
setkey(DT,ShortName)
DT <- DT[!"fiducial"]
...
suppressWarnings(print(p))
  ...
  ggtitle(paste("Loess Model of Spot Cell Count  for",unique(DT$CellLine), "cells in plate",unique(DT$Barcode)))+xlab("Spot Cell Count")+
...
for (barcode in unique(sDT$Barcode)){
  ...
  p <- ggplot(DT, aes(x=ArrayColumn, y=ArrayRow,colour=SpotCellCount))+
    ...
    scale_y_reverse()+   scale_x_continuous(breaks= c(min(DT$ArrayColumn),round(mean(c(min(DT$ArrayColumn),max(DT$ArrayColumn)))),max(DT$ArrayColumn)))+
    scale_colour_gradient(low = "white", high = "red")+
    guides(colour = guide_legend("CellMask", keywidth = .5, keyheight = .5))+
    ggtitle(paste("Spot Cell Count for",unique(DT$CellLine), "cells in plate",unique(DT$Barcode)))+
    xlab("")+ylab("")+theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=1),plot.title = element_text(size = rel(.5)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))+
  ...
  p <- ggplot(DT, aes(x=SpotCellCount))+
    ...
    ggtitle(paste("Spot Cell Count for",unique(DT$CellLine), "cells in plate",unique(DT$Barcode)))+xlab("CellMask")+
  ...
  p <- ggplot(DT, aes(x=LoessSCC))+
    ...
    ggtitle(paste("Loess Model of Spot Cell Count for",unique(DT$CellLine), "cells in plate",unique(DT$Barcode)))+xlab("CellMask")+xlim(0,3)+
    theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(.5)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(.5)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))+
    facet_wrap(~Well, ncol=4)
  suppressWarnings(print(p))
...
##Filtering
Remove wells A01 and A02 from further analysis due to quality issues in the cell level data.
...
This analysis shows the CVs of the Spot Cell Count as the number of MEP replicates increases from 3 to 13. The data is the median normalized SCC responses from well A03 in plate LI8X00101. The results are from 100 runs, each calculating CVs on randomly choosen replicates of 3-13 MEP responses. The intention is to show the range of CVs to expect if not all of the array is used in the downstream analysis.
...
```{r Create_LI8X00101_CVS_vs_Replicates, echo=FALSE, fig.width = 8, fig.height=4}
...
cvList <- lapply(3:min(table(DT$ShortName)), function(nrRep){
  ...
  cvDT <- DT[,evalCVs(SpotCellCount.MedNorm,repV),by="Well,ShortName"]
  setnames(cvDT, "V1", "CV")
  cvDT$Reps <- nrRep
  return(cvDT)
})
...
```{r Plot LI8X00101_CVS_vs_Replicates, echo=FALSE, fig.width = 8, fig.height=11}
...
```{r Create_LI8X00101_Median_vs_Replicates, echo=FALSE, fig.width = 8, fig.height=4}

permList <- lapply(1:100,function (x){
medList <- lapply(3:min(table(DT$ShortName)), function(nrRep){
  repV <- sample(1:min(table(DT$ShortName)),size = nrRep,replace = FALSE)
  medDT <- DT[,evalMedians(SpotCellCount.MedNorm,repV),by="Well,ShortName"]
...
p <- ggplot(medPerms[medPerms$ShortName %in% c("LAMB1", "DSG2")], aes(x=Reps,y=Median, colour = ShortName))+
  geom_point(alpha=.3)+
  #ylim(.5,1)+
#       p <- ggplot(lfDT, aes(x=ShortName))+
#   geom_bar(width=.8)+geom_hline(yintercept = mean(table(lfDT$ShortName)), colour="blue")+
...
```{r Plot LI8X00101_Meds_vs_Replicates, echo=FALSE, fig.width = 8, fig.height=11}
...
The fiducials in the 1,1 and 35,20 spots consist of Alexa 488 flurophores conjugated to gelatin (maybe). This analysis compares the spots printed after the fiducials to their replicates, looking for evidence of carryover that could skew the data. The post-fiducial spots do not appear to be different from their replicates (a statistical test will be performed to validate this). The lower plot shows that the normalized median and distribution of the fiducials are significantly hjigher than the ECM proteins. 
...
sDT$PostFid <- (sDT$Block == 28 & sDT$Row == 1 & sDT$Column == 1 & sDT$Well %in% c("B01","A04")) | (sDT$Block == 1 & sDT$Row == 5 & sDT$Column == 4 & sDT$Well %in% c("B01","A04"))

 #Limit on only the ECMs that are post fiducial
 postFidECMs <- unique(sDT$ShortName[sDT$PostFid])
 p <- ggplot(sDT[sDT$ShortName %in% postFidECMs], aes(x=factor(Block), y=Cyto.Mean.Intensity.Alexa.488.MedNorm, colour=PostFid))+
 ...
 print(p)
  ...
  p <- ggplot(sDT, aes(x=ShortName, y=Cyto.Mean.Intensity.Alexa.488.MedNorm, colour=sDT$ShortName=="fiducial"))+
   geom_boxplot(notch=FALSE)+
