...
cDT <- cDT[, MitoPerimeterRatio := calcGroupRatios(.SD,group="Perimeter",signal="Cyto.Mean.Intensity.Alexa.647.MedNorm"), by="Ligand,ECMp"]

cDT <- cDT[, MitoSparseRatio := calcGroupRatios(.SD,group="Sparse",signal="Cyto.Mean.Intensity.Alexa.647.MedNorm"), by="Ligand,ECMp"]
...
cDT <- cDT[, EdUPerimeterRatio := calcGroupRatios(.SD,group="Perimeter",signal="Mean.Intensity.Alexa.647.MedNorm"), by="Ligand,ECMp"]

cDT <- cDT[, EdUSparseRatio := calcGroupRatios(.SD,group="Sparse",signal="Mean.Intensity.Alexa.647.MedNorm"), by="Ligand,ECMp"]
...
p <- ggplot(cDT, aes(x=factor(Perimeter), y=log2(Mean.Intensity.Alexa.647.SpotNorm)))+geom_boxplot(notch=FALSE)+
  ylim(0,1)+
  facet_wrap(~ECMp)
  suppressWarnings(print(p))
  ...
  p <- ggplot(cDT, aes(x=factor(Sparse), y=log2(Mean.Intensity.Alexa.647.SpotNorm)))+geom_boxplot(notch=FALSE)+
  ylim(0,1)+
  facet_wrap(~ECMp)
  suppressWarnings(print(p))
...
The following plots look at the plate normalized responses of the H3 signal stratified by ligand, ECM protein and MEP.
...
Lowest 25 `r unique(mepDT$Endpoint488)` responses.  
`r mepDT[,list(Ligand,ECMp,Mean.Intensity.Alexa.488.MedNorm)][1:25] `

Highest 25 `r unique(mepDT$Endpoint488)` responses.  
  `r mepDT[,list(Ligand,ECMp,Mean.Intensity.Alexa.488.MedNorm)][(nrow(mepDT)-25):nrow(mepDT)] `
