---
title: "LINCs MEP SS1 Pilot Analysis Pipeline"
author: "Mark Dane"
date: "`r Sys.Date()`"
output: pdf_document
---
...
popDT <- fread("./SS1/Population/Annotated Data/PC3_SS1_PopAnn.txt")

cDT <- fread("./SS1/Cell/Annotated Data/PC3_SS1_CellAnn.txt")

sDT <- fread("./SS1/Cell/Annotated Data/PC3_SS1_CellSpotAnn.txt")
...
for (barcode in barcodes){
  ...
  p <- ggplot(DT, aes(x=Cyto.Mean.Intensity.Alexa.488))+
    geom_histogram(binwidth = 5)+
    ggtitle(paste(unique(DT$Endpoint488),"Signal in", unique(DT$CellLine), "cells in plate",unique(DT$Barcode)))+xlab("Cyto.Mean.Intensity.Alexa.488")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.5)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(.5)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))+
    facet_wrap(~Well, ncol=4)
  suppressWarnings(print(p))
  
    p <- ggplot(DT, aes(x=Cyto.Mean.Intensity.Alexa.555))+
    geom_histogram(binwidth = 5)+
    ggtitle(paste(unique(DT$Endpoint555),"Signal in", unique(DT$CellLine), "cells in plate",unique(DT$Barcode)))+xlab("Cyto.Mean.Intensity.Alexa.555")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.5)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(.5)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))+
    facet_wrap(~Well, ncol=4)
  suppressWarnings(print(p))
  
    p <- ggplot(DT, aes(x=Cyto.Mean.Intensity.Alexa.647))+
    geom_histogram(binwidth = 1)+
    ggtitle(paste(unique(DT$Endpoint647),"Signal in", unique(DT$CellLine), "cells in plate",unique(DT$Barcode)))+xlab("Cyto.Mean.Intensity.Alexa.647")+
  xlim(0,60)+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.5)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(.5)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))+
    facet_wrap(~Well, ncol=4)
  suppressWarnings(print(p))
