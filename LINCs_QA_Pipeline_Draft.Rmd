...
##Summary
This experiment uses the LINCs cell cycle staining set on PC3 cells in 8 well MEMAs. 
...
#popDT <- MEMA::popDT
popDT <- fread("./Annotated Data/LI8X00110_17PopAnn.txt")
...
```{r Plot LI8X001_CellMask_heatmaps , echo=FALSE, fig.width=2.5,fig.height=4}
DT <-popDT[popDT$Barcode=="LI8X00101" & popDT$Well == "A02"]
#Remove the fiducial entries
setkey(DT,ShortName)
DT <- DT[!"fiducial"]
...
suppressWarnings(print(p))
  ...
  ggtitle(paste("Loess Model of Tecan",unique(DT$EndPoint555)," Signal\n for",unique(DT$CellLine), "cells in plate",unique(DT$Barcode)))+xlab("CellMask")+
...
```{r Plot LI8X001_CellMask_Show_QA, echo=FALSE, fig.width=3.7,fig.height=4}
...
for (barcode in unique(popDT$Barcode)){
  ...
  p <- ggplot(DT, aes(x=Loess555))+
    ...
    geom_vline(xintercept=lthresh, colour="blue")+
    geom_text(data=wellScores, aes(label=paste0("QA\n",QAScore)), x = 1, y = 175, size = rel(3), colour="red")+
    ggtitle(paste("Loess Model of Tecan",unique(DT$EndPoint555)," Signal\n for",unique(DT$CellLine), "cells in plate",unique(DT$Barcode)))+xlab("CellMask")+
    theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(.5)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(.5)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))+
    facet_wrap(~Well, ncol=4)
  suppressWarnings(print(p))
...
```{r Plot LI8X00101_Median_vs_Replicates, echo=FALSE, fig.width = 8, fig.height=11}
...
#   p <- ggplot(popDT[popDT$ShortName %in% postFidECMs], aes(x=PostFid, y=Net.532.MedNorm))+
#    geom_boxplot(notch=TRUE)+
#    facet_wrap(~ShortName)
#  
#  print(p)
#  
#    p <- ggplot(popDT[popDT$ShortName %in% postFidECMs], aes(x=PostFid, y=Net.635.MedNorm))+
#    geom_boxplot(notch=TRUE)+
#    facet_wrap(~ShortName)
#  
#  print(p)
